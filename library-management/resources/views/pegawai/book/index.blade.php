...
<div class="p-4 sm:ml-64 relative overflow-x-auto shadow-md sm:rounded-lg mt-5 bg-white" style="border-radius: 25px; margin-right: 25px; margin-bottom: 25px; min-height: 100vh;">
    <div class="flex justify-between items-center pb-5 dark:bg-gray-900 mb-5">
        ...
        <!-- Search Bar -->
        <form action="{{ route('employee.book.search') }}" method="GET" class="relative flex items-center justify-start max-w-[400px] w-full mt-1 ml-0">
            <input 
                type="text" 
                name="query" 
    ...
    <div class="flex justify-between items-center pb-4">
        ...
        <!-- Filters and Sorting -->
        <div class="flex items-center gap-4 mt-4 lg:mt-0 align-items: center">
            <!-- Filter Kategori Buku -->
            <form action="{{ route('employee.book.index') }}" method="GET" class="w-full md:w-auto">
                <select 
                    name="kategori" 
            ...
            <form action="{{ route('employee.book.index') }}" method="GET" class="w-full md:w-auto">
                <select 
                    ...
                    style="border: none; box-shadow: none; background-color:#a2bcd7;" 
                    onchange="this.form.submit()">
                    <option value="" class="text-white">Sort by Stock</option>
                    <option value="stok_asc" {{ request()->query('sort') == 'stok_asc' ? 'selected' : '' }}>Stock (Low to High)</option>
                    <option value="stok_desc" {{ request()->query('sort') == 'stok_desc' ? 'selected' : '' }}>Stock (High to Low)</option>
                </select>
            </form>
        </div>
    </div>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase text-center dark:bg-gray-700 dark:text-gray-400" style="background-color: #e6edf5;">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">Tahun Penerbit</th>
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-center">
                ...
                <td class="px-6 py-4 flex justify-center items-center gap-4 h-full" style="margin-top: 52px;">
                    <a href="{{ route('employee.book.edit', $buku) }}" class="text-yellow-400 hover:text-yellow-600 focus:outline-none">
                        <i class="fas fa-edit text-lg"></i>
                    </a>
                    <form method="POST" action="{{ route('employee.destroy', $buku->id) }}" onsubmit="return confirm('Apakah kamu yakin ingin menghapus data?')" class="d-inline">
                        @csrf
                        @method('DELETE')
