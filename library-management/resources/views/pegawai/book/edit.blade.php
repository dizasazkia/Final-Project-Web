...
<div class="p-4 sm:ml-64 relative overflow-x-auto shadow-md sm:rounded-lg mt-5 bg-white" style="border-radius: 25px; margin-right: 25px; margin-bottom: 25px; min-height: 100vh;">
    <form class="mx-auto mt-14 p-6 shadow-lg bg-gray-50 rounded-xl" action="{{ route('employee.book.update', $book->id) }}" method="POST" enctype="multipart/form-data" style="border-radius: 25px; margin-bottom: 25px; width: 800px;">
        @csrf
        @method('PUT')
        ...
        <!-- Input Fields for Judul, Penulis, Penerbit, Tahun Terbit -->
        ...
        <!-- Input Fields for Stok and Deskripsi -->
        @foreach(['stok', 'deskripsi'] as $field)
            <div class="relative z-0 w-full mb-5 group">
                @if ($field == 'stok')
                    <input type="number" name="{{ $field }}" id="{{ $field }}" value="{{ old($field, $book->$field) }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                @else
                    <textarea name="{{ $field }}" id="{{ $field }}" rows="4" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" ">{{ old($field, $book->$field) }}</textarea>
                @endif
                <label for="{{ $field }}" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                    {{ ucfirst(str_replace('_', ' ', $field)) }}
                </label>
