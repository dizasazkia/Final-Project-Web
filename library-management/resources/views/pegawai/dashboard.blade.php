...
<div class="p-4 sm:ml-64 relative overflow-x-auto shadow-md sm:rounded-lg mt-5 bg-white" style="border-radius: 25px; margin-right: 25px; margin-bottom: 25px; min-height: 100vh;">
    ...
    <div class="flex items-center justify-between ml-3 mr-3 mb-5 shadow-sm"
        ...
        <div>
            ...
            <p class="font-medium text-white mt-3" style="font-size: 18px;">
                This is your command center to monitor and manage the library system. <br>
                Use the tools provided to ensure smooth operations and keep things in check. <br>
                You're in charge of making sure everything runs smoothly.
            </p>
        </div>
        <!-- Gambar -->
        <div>
    ...
    <div class="grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 10px; margin-bottom: 50px;">
        ...
        <!-- Buku Menunggu Konfirmasi -->
        <div class="card ml-3" style="background-color: #f9fafb; padding: 20px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); max-width: 400px; margin-left: -100px;">
            <div class="flex" style="display: flex; align-items: center;">
                <div class="icon" style="font-size: 50px; color: #ffcc00; margin-right: 30px;">
                    <i class="fas fa-exclamation-circle" style="font-size: 50px;"></i> 
                </div>
                <div>
                    <h3 style="font-size: 18px; font-weight: bold; color: #3d426c;">Books Awaiting Confirmation</h3>
                    <p style="font-size: 24px; font-weight: bold; color: #4e4e4e;">{{ $pendingReturns }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6 ml-3 mr-3 mb-5">
        ...
        <div class="bg-white p-5 rounded-lg shadow-md">
            <h3 class="font-bold text-xl mb-3 text-gray-800">Statistik Peminjaman per Hari</h3>
            <canvas id="loansPerDayChart" width="400" height="200"></canvas>            
        </div>
    </div>
</div>
...
<script>
    ...
    // Tambahkan script untuk grafik Statistik Peminjaman
    var ctx3 = document.getElementById('loansPerDayChart').getContext('2d');
    var loansPerDayChart = new Chart(ctx3, {
        type: 'line',
        data: {
